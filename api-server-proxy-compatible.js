...
const app = express();
// Utiliser le port 3000 pour être compatible avec la configuration du frontend
...
app.get('/', (req, res) => {
  res.json({
    status: 'success',
    message: 'API PPT Template Manager - Proxy Compatible - Prête',
    version: '1.0.0',
    endpoints: [
...
function handleCategories(req, res) {
  const categories = [
    { id: 1, name: 'Commercial', icon: 'business', color: '#4A90E2' },
    { id: 2, name: 'Finance', icon: 'monetization_on', color: '#E24A78' },
    { id: 3, name: 'Formation', icon: 'school', color: '#4AE278' },
    { id: 4, name: 'Marketing', icon: 'trending_up', color: '#E2CF4A' }
  ];
  res.json(categories);
}

function handleTemplates(req, res) {
  // Récupérer les templates uploadés
  const uploadedTemplates = getUploadedTemplates();
...
function handleAddTemplate(req, res) {
  ...
  const newTemplate = {
    ...
    thumbnail: `https://placehold.co/300x200/4A90E2/FFFFFF?text=${encodeURIComponent(id.toString().substring(0, 10))}`,
  ...
  // Détecter si la requête vient du endpoint /upload
  if (req.originalUrl.includes('/upload')) {
    // Retourner directement le template pour le endpoint /upload
    console.log('Endpoint /upload détecté, retour du template directement');
    res.status(201).json(newTemplate);
  } else {
    // Format standard avec status et message pour les autres endpoints
    console.log('Endpoint standard détecté, retour avec structure standard');
    res.status(201).json({
      status: 'success',
...
function handleDeleteTemplate(req, res) {
  ...
  // Récupérer tous les templates uploadés
  const uploadedTemplates = getUploadedTemplates();
  
  // Chercher le template avec l'ID spécifié
  const uploadedTemplate = uploadedTemplates.find(t => t.id === id);
  
  if (uploadedTemplate) {
    // Supprimer le fichier physique
    try {
      // Extraire le nom du fichier à partir du chemin du fichier
      const fileName = path.basename(uploadedTemplate.file_path);
...
function getUploadedTemplates() {
  ...
  if (fs.existsSync(uploadDir)) {
    ...
    uploadedTemplates = files
      .filter(file => path.extname(file).toLowerCase() === '.pptx')
      .map(file => {
        const stats = fs.statSync(path.join(uploadDir, file));
        const name = path.basename(file, path.extname(file));
        
        // Extraire l'ID du timestamp dans le nom du fichier
        const idMatch = name.match(/^(\d+)/);
        const id = idMatch ? parseInt(idMatch[1]) : Date.now();
        
        return {
          id: id,
          name: name.replace(/^\d+-/, ''), // Enlever le préfixe timestamp
          description: ``,
          thumbnail: `https://placehold.co/300x200/4AE2B5/FFFFFF?text=${encodeURIComponent(id.toString().substring(0, 10))}`,
          category_id: 1,
          folder_id: 1,
...
app.listen(PORT, () => {
  console.log(`
✅ Serveur API Proxy-Compatible démarré sur http://localhost:${PORT}
...
🛠️ Améliorations:
   - Configuration adaptée au frontend (port 3000)
   - Format de réponse adapté selon le endpoint appelé
   - Gestion correcte des templates uploadés avec IDs timestamp
   - Suppression de templates fonctionnelle
  `);
});
