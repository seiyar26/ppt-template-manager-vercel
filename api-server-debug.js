...
app.get('/', (req, res) => {
  res.json({
    status: 'success',
    message: 'API PPT Template Manager - Mode Debug',
    version: '1.0.0',
    endpoints: [
      '/health',
      '/auth/login',
...
// Route pour l'authentification avec différents formats pour tester
...
function handleLogin(req, res) {
  ...
  console.log(`\n🔑 Tentative de connexion avec: ${email}, password: ${password}`);
  ...
  if (email === 'admin@admin.com' && password === 'admin123') {
    ...
    console.log('📤 Envoi de la réponse avec token:', responseData);
...
function handleCurrentUser(req, res) {
  ...
  if (token) {
    console.log('🔓 Token reçu, utilisateur authentifié:', token);
    return res.json({
      status: 'success',
      data: {
        user: {
...
app.listen(PORT, () => {
  console.log(`
✅ Serveur API en MODE DEBUG démarré sur http://localhost:${PORT}
📡 API accessible sur http://localhost:${PORT}/api et http://localhost:${PORT}/
🔐 Identifiants de connexion: admin@admin.com / admin123
🔍 Log détaillé activé pour déboguer l'authentification
  `);
});
