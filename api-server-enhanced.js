...
function handleAddTemplate(req, res) {
  ...
  // Format attendu par le frontend - avec status et message
  res.status(201).json({
    status: 'success',
...
app.listen(PORT, () => {
  console.log(`
✅ Serveur API amélioré démarré sur http://localhost:${PORT}
...
🛠️ Améliorations:
   - Correction de la suppression des templates uploadés
   - Format de réponse uniforme pour les templates
   - Extraction correcte des IDs des templates uploadés
  `);
});
